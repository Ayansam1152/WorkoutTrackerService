...
namespace WorkoutTrackerServices.Controllers
{
    ...
    public class WorkoutController : ControllerBase
    {
        ...
        /// <summary>
        /// Chat with LLM (Ollama, Llama 3, etc.) using OllamaSharp.
        /// </summary>
        /// <param name="request">Prompt and optional model name.</param>
        /// <returns>LLM response.</returns>
        [HttpPost("llm/chat")]
        [ProducesResponseType(typeof(ApiResponse<LlmResponseDto>), 200)]
        [ProducesResponseType(typeof(ApiResponse<string>), 500)]
        public async Task<ActionResult<ApiResponse<LlmResponseDto>>> ChatWithLlm([FromBody] LlmRequestDto request)
        {
            try
            {
            ...
            {
                logger.LogError(ex, "LLM chat failed.");
                return StatusCode(StatusCodes.Status500InternalServerError, new ApiResponse<string> { Data = [$"LLM error: {ex.Message}"] });
            }
        }
